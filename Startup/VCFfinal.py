        ...
        try:
            ...
            if not vsp_control_plane_ip:
                ...
                def apply_proxy_to_nodes(node_ips):
                    ...
                    NO_PROXY_PARTS = [
                        'localhost', '127.0.0.1', '10.0.0.0/8', '10.96.0.0/12', 
                        '172.16.0.0/16', '192.168.0.0/16', '198.18.0.0/16', 
                        '.site-a.vcf.lab', '.site-b.vcf.lab', '.vcf.lab', 
...
'''
    ...
    if not dry_run:
        ...
        try:
            # Ensure VCFA node is uncordoned
            cmd = f"sshpass -p '{password}' ssh -o StrictHostKeyChecking=no vmware-system-user@10.1.1.70 \"echo '{password}' | sudo -S kubectl --kubeconfig=/etc/kubernetes/admin.conf uncordon auto-platform-a-b7nps 2>/dev/null\""
            subprocess.run(cmd, shell=True, check=False)
            
            # Scale critical statefulsets to 1
            cmd = f"sshpass -p '{password}' ssh -o StrictHostKeyChecking=no vmware-system-user@10.1.1.70 \"echo '{password}' | sudo -S kubectl --kubeconfig=/etc/kubernetes/admin.conf scale statefulset rabbitmq-ha tenant-manager vco-app -n prelude --replicas=1 2>/dev/null\""
            subprocess.run(cmd, shell=True, check=False)
            
            # Scale all 0-replica deployments to 1
            cmd = f"sshpass -p '{password}' ssh -o StrictHostKeyChecking=no vmware-system-user@10.1.1.70 \"echo '{password}' | sudo -S kubectl --kubeconfig=/etc/kubernetes/admin.conf get deployments -n prelude -o json\""
            ...
            if '{' in out:
                ...
                for d in data.get('items', []):
                    ...
                    if d['spec'].get('replicas', 1) == 0:
                        ...
                        cmd_scale = f"sshpass -p '{password}' ssh -o StrictHostKeyChecking=no vmware-system-user@10.1.1.70 \"echo '{password}' | sudo -S kubectl --kubeconfig=/etc/kubernetes/admin.conf scale deployment {name} -n prelude --replicas=1\""
                        subprocess.run(cmd_scale, shell=True, check=False)
        except Exception as e:
